...
from generic_config import FAILED_URLS_FILENAME, SKU_FILE_NAME
...
def generate_csv_report(records: List[FinalProductRecord], output_path: Path) -> None:
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["SKU", "Price", "Availability", "URL", "Timestamp", "Detection_Status"])
        writer.writerows([
            (r.normalized_sku, r.price, r.availability_code, r.url, r.timestamp.isoformat(), r.detection_status)
            for r in records
        ])
...
def log_failed_urls(failed_urls: List[Tuple[str, str, str]]) -> None:
    ...
    with open(FAILED_URLS_FILENAME, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["URL", "Reason", "Details"])
        writer.writerows(failed_urls)
    logger.info(f"Failed URLs logged to {FAILED_URLS_FILENAME.name}")
